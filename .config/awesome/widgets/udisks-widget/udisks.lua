...
local function poweroff_device(device)
	if device.Mounted == false then
		ret, err = system_bus:call(
			'org.freedesktop.UDisks2',
			--'/org/freedesktop/UDisks2/drives/' .. device.Drive,
			device.Drive,
	...
	for device, data in pairs(devices) do
		if data.Device and data.OK then
			...
			row:buttons(awful.util.table.join(
				...
				awful.button({ }, 2, function () poweroff_device(data) end),
