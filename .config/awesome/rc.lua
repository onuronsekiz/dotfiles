...
awful.util.mymainmenu = freedesktop.menu.build({
    ...
    after = {
        { "Terminal", terminal, "/usr/share/icons/hicolor/scalable/apps/Alacritty.svg" },
        -- other triads can be put here
    }
})
...
globalkeys = my_table.join(
	...
	awful.key({}, "Print",
		function()
			awful.spawn("flameshot full -p Pictures/")--.. pictures_path)
		end, {description = "Screenshot to File", group = "hotkeys"}),
	awful.key({altkey}, "Print",
		function()
			awful.spawn("flameshot gui -p Pictures/")--" .. pictures_path)
		end, {description = "Screenshot Select to File", group = "hotkeys"}),
	awful.key({"Control"}, "Print",
		function()
			awful.spawn("flameshot full -c")
		end, {description = "Screenshot Select to Clipboard", group = "hotkeys"}),
	...
	awful.key({modkey}, "r",
		...
		end, {description = "run command", group = "awesome"})
)

clientkeys = my_table.join(
	awful.key({modkey,altkey}, "Up",
		function (c)
			move_client_to_screen(c, gears.math.cycle(screen:count(), c.screen.index+1))-- end
	...
	awful.key({modkey,altkey}, "Left",
		function (c)
			c:move_to_tag(c.screen.tags[gears.math.cycle(#c.screen.tags, c.first_tag.index - 1)]) awful.tag.viewprev()
...
for i = 1, 9 do
	...
	awful.key({ modkey, "Control" }, "#" .. i + 9,
		...
		end, {description = "focus to screen #" .. i, group = "tag"})
	)
	clientkeys = my_table.join(clientkeys,
	-- move client to tags
	...
	awful.key({ modkey, altkey, "Control" }, "#" .. i + 9,
		...
		end, {description = "move to screen #" .. i, group = "clients movement"})
	)
end
...
awful.rules.rules = {
	...
	{rule = {name = "Formatting X-Y Plot" }, properties = { floating = true, maximize = false, below = false, ontop = true }},
	{rule = {name = "Series Collection Editor" }, properties = { floating = true, maximize = false, below = false, ontop = true }},
	{rule = {class = "gomatrix", name = "Alacritty" }, properties = {floating = true, fullscreen = true, below = true, skip_taskbar = true, sticky = true, opacity=0.25, border_width = 0, focusable = false, ontop = false, dockable = true }},
	--{rule = {class = "Gimp", name = "*"}, properties = { floating = false, maximize = false, below = true }},
...
awful.spawn.easy_async_with_shell("pactl info &")
os.execute("picom -b")
awful.spawn.easy_async_with_shell("pulseeffects --gapplication-service &")
os.execute("cmst -m &")